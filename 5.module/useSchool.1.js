...
Module.prototype.load = function(filename){ //{filename:'c://xxxx',exports:'yyccQQu'}
    // 模块可能是json 也有可能是js
...
function req(filename){ // filename是文件名 文件名可能没有后缀
    ...
    module.load(filename); // 加载这个模块     {filename:'xxx',exports = 'yyccQQu'}
    Module._cache[filename] = module;
    module.loaded = true; // 表示当前模块是否加载完 
    return module.exports;
}
let result = require('./school');
result = req('./school'); // 手写node require方法
console.log(result);
